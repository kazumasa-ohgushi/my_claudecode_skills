...
```
/md-to-gdoc <path/to/file.md> [--title "Document Title"]
```
...
2. **Locate the Python script** — it is bundled alongside this SKILL.md file:
   Use Glob to find `md_to_gdoc.py` by searching `**/.claude/skills/_unclassified/md-to-gdoc/md_to_gdoc.py` in the home directory, or locate it relative to this SKILL.md's own path.
...
3. **Run the script** using an available Python 3.12+ interpreter.
   ...
   ```bash
   python3 /absolute/path/to/md_to_gdoc.py <absolute_path_to_md> [--title "Title"]
   ```
